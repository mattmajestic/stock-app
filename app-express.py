...
def get_moving_averages():
    data = get_data()  # Assuming get_data() fetches stock price history
    short_term_window = 50
    long_term_window = 200
    
    # Calculate moving averages
    data['ShortTerm_MA'] = data['Close'].rolling(window=short_term_window).mean()
    data['LongTerm_MA'] = data['Close'].rolling(window=long_term_window).mean()
    
    return data[['ShortTerm_MA', 'LongTerm_MA']]
